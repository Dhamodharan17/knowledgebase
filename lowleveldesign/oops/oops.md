...
- **"Has-a" or "Uses-a" Relationships:** Avoid inheritance when the relationship is "has-a" or "uses-a" rather than "is-a". A `Car` _has an_ `Engine`, it is not an `Engine`. A `Printer` _uses a_ `Logger`, it is not a `Logger`.
